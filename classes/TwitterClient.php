...
class TwitterClient
{
     ...
     */
    public function homeTimeline($count = 10)
    {
        $cacheKey = 'rainlab-twitter-home_timeline';
        $cached = Cache::get($cacheKey, false);
        if ($cached && ($unserialized = @unserialize($cached)) !== false)
            return $unserialized;
        ...
        $code = $obj->client->user_request(array(
            'url'    => $obj->client->url('1.1/statuses/home_timeline.json'),
            'params' => array(
                'include_entities' => true,
                'count'            => $count
            )
        ...
        foreach ($result as &$message) {
            ...
            $text = preg_replace('/\#\w+/', '<span class="tag">$0</span>', $text);
            $message['text_processed'] = $obj->urlsToLinks($text);
            $message['date_processed'] = $obj->diffForHumans($message['created_at']);
        }
    ...
    public function updateStatus($status)
    {
        ...
        $code = $obj->client->user_request(array(
            'method' => 'POST',
            'url'    => $obj->client->url('1.1/statuses/update.json'),
            'params' => array(
                'status' => $status
            )
        ));
